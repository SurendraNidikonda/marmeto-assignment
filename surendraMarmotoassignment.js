...
const fetchData = async (category) => {
    try {
        ...
        const List = filteredList[0].category_products;

        const currentData = List.map((each) => ({
...
const removePrevousCategory = (category) => {
    const {
        data
    } = state;
...
const updateState = (currentCategory) => {
...
const handleButtonClick = async (event) => {
    ...
    removePrevousCategory(state.category);
    updateState(currentCategory);
    app();
};

const createAndAppendItem = (cardDetails) => {
